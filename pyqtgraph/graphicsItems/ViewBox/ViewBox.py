    ...
    * Item coordinate mapping methods
    ...
    def _invertAxis(self, ax, inv):
        ...
        if ax:
            ...
            self.sigXRangeChanged.emit(self, tuple(self.viewRange()[ax]))
    ...
    def childrenBounds(self, frac=None, orthoRange=(None,None), items=None):
        ...
        for item in items:
            ...
            if hasattr(item, 'dataBounds'):
                ...
                xr = (bounds.x(), bounds.x() + bounds.width())
                yr = (bounds.y(), bounds.y() + bounds.height())
                with np.errstate(invalid='ignore', divide='ignore'):
                    if self.state['xLog']:
                        xr = [x if np.isfinite(x) else 0 for x in np.log10(xr)]
                    if self.state['yLog']:
                        yr = [x if np.isfinite(x) else 0 for x in np.log10(yr)]
                bounds = QtCore.QRectF(xr[0], yr[0], xr[1]-xr[0], yr[1]-yr[0])
                bounds = self.mapFromItemToView(item, bounds).boundingRect()
